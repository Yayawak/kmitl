...
public class Main {
    ...
    static  int[][] markMap(int[][] mineMap)
    {
        ...
        // i ended at 11
        // j ended at 7
        for (int i = 0; i < mineMap.length - 1; i++)
        {
            // if (i == 0 || i == mineMap[0].length - 2) continue;
            for (int j = 0; j < mineMap[0].length - 1; j++)
            {
                // if (j == 0 || j == oneTileExtendMap[0].length - 2) continue;
                // todo : check adjacent tile and filter to sum
                oneTileExtendMap[i+1][j+1] = mineMap[i][j];

                // if (i != 0 || i != oneTileExtendMap.length - 1
                //     || j != 0 || j != oneTileExtendMap[0].length - 1
                // )
                try {
                    // * pivot index = (x=i-1, y=j-1)
                    int count_mid = 0;
                    ...
                    for (int x = i-1; x < i+3; x++)
                    {
                        ...
                        {
                            if (oneTileExtendMap[x][y] == 9) count_mid++;
                            // if (mineMap[x][y] == 9) count_mid++;
                        }
                    }
                    to_mark_map[i][j-1] = count_mid;
