...
public class CircleAlgorithms extends Drawing {
    private static Point[] getOctancesFromPoint(Point p)
    {
        int x = p.x;
        int y = p.y;
    ...
    {
        return midpointDrawCirlcleWithStrokeSize(X, Y, r, 1);
    }

    public static List<Point> midpointDrawCirlcleWithStrokeSize(int X, int Y, int r, int strokeSize)
    {
        ...
        while (x <= y)
        {
            ...
            for (var p : oct)
            {
                ...
                // plotDot(p.x, p.y);
                drawGeneralDot(p.x, p.y, strokeSize);
                // ret[k] = p;
                // k++;
                ret.add(p);
            }
